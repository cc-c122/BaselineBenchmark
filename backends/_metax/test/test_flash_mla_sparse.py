...
pytestmark = pytest.mark.skipif(
    not is_metax_available(), reason="sparse FlashMLA tests require MetaX PyTorch"
)
...
class FlashmlaSparseTestKit:
    ...
    def get_correctness_test_params():
        return [
            Flashmla_Sparse_Test_Param(1, 1024, 128, 64, 1, 576, 512),
            Flashmla_Sparse_Test_Param(16, 2048, 256, 128, 1, 576, 512),
        ]
...
def test_flashmla_sparse(param):
    ...
    your_output, your_max_logbits, your_lse = flash_mla_sparse_fwd(
        q,
        kv,
        indices,
        sm_scale,
    ...
    torch.testing.assert_close(
        your_output, ref_output, atol=1e-2, rtol=3.01 / 128
    )
    torch.testing.assert_close(
        your_max_logbits, ref_max_logbits, atol=1e-4, rtol=2.01 / 65536
    )
    torch.testing.assert_close(
        your_lse, ref_lse, atol=1e-4, rtol=2.01 / 65536
    )
